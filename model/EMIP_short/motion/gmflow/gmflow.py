...
class GMFlow(nn.Module):
    def __init__(self,
        ...
        self.attn_splits_list = args['GMFlow']['attn_splits_list']
        self.corr_radius_list = args['GMFlow']['corr_radius_list']
        self.prop_radius_list = args['GMFlow']['prop_radius_list']
        self.pred_bidir_flow = args['GMFlow']['pred_bidir_flow']
    ...
    def forward(self, feature0_list, feature1_list):
        ...
        for scale_idx in range(self.num_scales):
            ...
            # Transformer
            feature0, feature1 = self.transformer(feature0, feature1, attn_num_splits=attn_splits)  # 2 128 44 44

            # correlation and softmax
            if corr_radius == -1:  # global matching
                flow_pred, prob, corr = global_correlation_softmax(feature0, feature1, self.pred_bidir_flow)
            else:  # local matching
                flow_pred, prob, corr = local_correlation_softmax(feature0, feature1, corr_radius)

            # flow or residual flow
            flow = flow + flow_pred if flow is not None else flow_pred
            flow_ori.append(flow)
        ...
        for flow in flow_preds:
            flow_list = torch.split(flow, flow.size(0)//2, dim=0)
            flow_fw.append(flow_list[0])
            flow_bw.append(flow_list[1])

        for flow in flow_ori:
            flow_list_ori = torch.split(flow, flow.size(0)//2, dim=0)
            flow_fw_ori.append(flow_list_ori[0])
            flow_bw_ori.append(flow_list_ori[1])

        return flow_fw, flow_bw, corr
