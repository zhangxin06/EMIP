...
class TransformerLayer(nn.Module):
    def __init__(self,
        ...
        # 加adaptor  10log做法
        self.adaptor_fc1 = nn.Linear(128,int(128*0.25))
        self.adaptor_fc2 = nn.Linear(int(128*0.25),128)
        self.adaptor_relu = nn.ReLU()
    ...
    def forward(self, source, target,
        ...
        # source, target: [B, L, C]
        query, key, value = source, target, target # 都是(2*bs,1936,768)
        ...
        # 加adaptor d
        # message = self.adaptor_fc1(message)
        # message = self.adaptor_relu(message)
        # message = self.adaptor_fc2(message)
        # return source + message
...
class COD_TransformerLayer(nn.Module):
    def __init__(self,
                 d_model=256,
                 nhead=1,
                 attention_type='swin',
        ...
        super(COD_TransformerLayer, self).__init__()
...
class COD_TransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        super(COD_TransformerBlock, self).__init__()

        self.self_attn = COD_TransformerLayer(d_model=d_model,
                                          nhead=nhead,
        ...
        self.cross_attn_ffn = COD_TransformerLayer(d_model=d_model,
                                               nhead=nhead,
                                               attention_type=attention_type,
                                               ffn_dim_expansion=ffn_dim_expansion,
                                               with_shift=with_shift,
...
class FeatureFlowAttention(nn.Module):
    ...
    def forward(self, feature0, flow,
        ...
        if local_window_attn: # 不进
            return self.forward_local_window_attn(feature0, flow,
                                                  local_window_radius=local_window_radius)

        b, c, h, w = feature0.size() # (2*bs,128,44,44)
...
class COD_FeatureTransformer(nn.Module):
    def __init__(self,
                 num_layers=6,
                 d_model=128,
                 nhead=1,
        ...
        super(COD_FeatureTransformer, self).__init__()
        ...
        self.layers = nn.ModuleList([
            COD_TransformerBlock(d_model=d_model,
                             nhead=nhead,
                             attention_type=attention_type,
                             ffn_dim_expansion=ffn_dim_expansion,
                             with_shift=True if attention_type == 'swin' and i % 2 == 1 else False,
