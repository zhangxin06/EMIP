...
def eval_engine_moca(opt, txt_save_path):
    ...
    for _data_name in opt.data_lst[0]:
        ...
        with open(filename, 'a+') as file_to_write:
            ...
            for _model_name in opt.model_lst:
                ...
                pred_src = os.path.join(opt.pred_root, opt.model_typename, _model_name, _data_name)
                ...
                for case in case_list:
                    if 'VPS' in _data_name:
                        case_gt_name_list = glob.glob(gt_src + '/{}/*.png'.format(case))

                        case_gt_name_list.sort(
                            key=lambda name: (str(case), str(name.split('/')[-1].rstrip('.png')))
                        )

                        # for fair comparison, we remove the first frame and last frame in the video suggested by
                        ...
                        case_pred_name_list = [gt.replace(gt_src, pred_src).replace('/GT', '') for gt in
                                               case_gt_name_list]
                    else:
                        case_gt_name_list = glob.glob(gt_src + '/{}/GT/*.png'.format(case))

                        case_gt_name_list.sort(
                            key=lambda name: (str(case), int(name.split('/')[-1].rstrip('.png').split('_')[-1]))
                        )

                        # for fair comparison, we remove the first frame and last frame in the video suggested by
                        # reference: Shifting More Attention to Video Salient Object Detection
                        # https://github.com/DengPingFan/DAVSOD/blob/master/EvaluateTool/main.m
                        if 'CAD' in _data_name:
                            case_gt_name_list = case_gt_name_list[:]
                        else:
                            case_gt_name_list = case_gt_name_list[:-2]
                        case_pred_name_list = [gt.replace(gt_src, pred_src).replace('/GT', '') for gt in case_gt_name_list]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        nargs='+', action='append',
        choices=['MoCA', 'CAD_eval', 'CAD', 'MoCA_test', 'MoCA_train', 'MoCA_pseudo_test', 'MoCA-pseudo_train',
                 'VPS', 'VPS_EASY', 'VPS_HARD'])
    parser.add_argument(
        '--model_typename', type=str, help='the type of model',
        default='SAM')

    opt = parser.parse_args()

    txt_save_path = 'E:/Projects/EMIP/result/eval-result/{}/'.format(opt.model_typename)
    os.makedirs(txt_save_path, exist_ok=True)

    # start eval engine
    eval_engine_moca(opt, txt_save_path)