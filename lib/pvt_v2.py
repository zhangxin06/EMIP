...
class Mlp(nn.Module):
    ...
    def forward(self, x, H, W): # (bs,7744,64), H:88, W:88
        x = self.fc1(x) # (bs,7744,256)
        if self.linear:
            x = self.relu(x)
        x = self.dwconv(x, H, W) # (bs,7744,256)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x) # (bs,7744,64)
        x = self.drop(x)
        return x
...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, H, W = patch_embed(x) # (bs,3,352,352) -> (bs,7744,64), H:88, W:88
            for blk in block:
                x = blk(x, H, W)
            x = norm(x)
