...
class DGNet(nn.Module):
    ...
    def forward(self, x):  # bs,3,352,352
        ...
        xr3 = self.dr3(x3)  # bs,32,44,44
        xr4 = self.dr4(x4)  # bs,32,22,22
        xr5 = self.dr5(x5)  # bs,32,11,11

        # spatial path (encoder)
        xg, pg = self.texture_encoder(x)  # bs,32,44,44  bs,1,44,44 

        # decoder
        zt3, zt4, zt5 = self.git(xr3, xr4, xr5, xg)  # bs,32,44,44/ bs,32,22,22/ bs,32,11,11
